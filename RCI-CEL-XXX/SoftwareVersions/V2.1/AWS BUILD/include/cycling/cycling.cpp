...
void cycleComponentValues(bool continuousUpdate) {
  ...
  Serial.println("SIGNAL COUNTER: " + signalCounter);
  Serial.println("NUMBER OF REMOTE UNITS: " + numberOfRemoteUnits);
  if (!heartbeatCheckAlreadySetFailDefaults[signalCounter]) {
    signalStrengthString = String("OK");
    M5.Lcd.setTextColor(GREEN);
  } else {
    signalStrengthString = String("FAIL");
  ...
  if (isHost) {
    ...
    M5.Lcd.drawString("R" + stringTargetUnitID + String(" ") + signalStrengthString, 110, 210, 1);
  } else {
    M5.Lcd.drawString("H" + String(" ") + signalStrengthString, 125, 210, 1);
  }
