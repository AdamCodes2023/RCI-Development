...
void onMqttMessage(int messageSize) {
  ...
  */
  ...
  if (topic.substring(topic.length() - 10, topic.length()).equalsIgnoreCase("configDump")) {
    if (message.toInt() == 1) {
      publishConfigDumpFlag = true;
    }
  }
  ...
  if (topic.substring(topic.length() - 4, topic.length()).equalsIgnoreCase("RSSI")) {
    if (message.toInt() == 1) {
      publishModemRSSIFlag = true;
    }
  }

  if (topic.substring(topic.length() - 9, topic.length()).equalsIgnoreCase("logUpload")) {
    ...
    if (message.toInt() == 1) {
      logUploadStep1 = true;
      logUploadStep1PubOnce = true;
    }
    if (message.toInt() == 3 && logUploadStep1) {
      logUploadStep2 = true;
      logUploadStep2PubOnce = true;
    }
    if (message.equalsIgnoreCase("ON") && logUploadStep2) {
      clearUpdateFlags();
      loggingEnabled = true;
      logEnabledEvent();
      publishLoggingStatus = true;
    }
    if (message.equalsIgnoreCase("OFF") && logUploadStep2) {
      clearUpdateFlags();
      loggingEnabled = false;
      logEnabledEvent();
      publishLoggingStatus = true;
    }
    if (message.length() > 4 && logUploadStep2) {
      logUploadStep3 = true;
      String tempLogYear = message.substring(0, message.indexOf("/"));
      String tempLogMonth = message.substring(message.indexOf("/") + 1, message.length());
      uploadLogFileName = serialNumber + "_log_" + tempLogYear + "_" + tempLogMonth + ".csv";
    }
  }

  if (topic.substring(topic.length() - 9, topic.length()).equalsIgnoreCase("logDelete")) {
    if (message.toInt() == 1) {
      logDeleteStep1 = true;
      logDeleteStep1PubOnce = true;
    }
    if (message.toInt() == 3 && logDeleteStep1) {
      logDeleteStep2 = true;
      logDeleteStep2PubOnce = true;
    }
    if (message.length() > 4 && logDeleteStep2) {
      logDeleteStep3 = true;
      String tempLogYear = message.substring(0, message.indexOf("/"));
      String tempLogMonth = message.substring(message.indexOf("/") + 1, message.length());
      deleteLogFileName = "/" + serialNumber + "_log_" + tempLogYear + "_" + tempLogMonth + ".csv";
    }
  }
