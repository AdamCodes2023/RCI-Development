...
void publishCreds(bool final) {
  ...
  */
  ...
  if (receivedCredsFTP) {
    payload = "CREDENTIALS UPDATE SUCCESSFUL!";
  }
...
void publishConfigDump() {
  ...
  payload = "START CONFIG\nGROUP ID: " + groupID + "\n" + "UNIT ID: " + unitID + "\n" + "I/O Number: " + IONum + "\n" + "UPDATE TIME (MS): " + updateTime + "\n" + "UNIT NAME: " + consumerName + "\n" +
  "DO1 FEED: " + do1_feed + "\n" + "DO2 FEED: " + do2_feed + "\n" + "DO3 FEED: " + do3_feed + "\n" + "DO4 FEED: " + do4_feed + "\n" + "DO5 FEED: " + do5_feed + "\n" + "DO6 FEED: " + do6_feed + "\n" + "DO7 FEED: " + do7_feed + "\n" + "DO8 FEED: " + do8_feed + "\n" +
  "AO1 FEED: " + ao1_feed + "\n" + "AO2 FEED: " + ao2_feed + "\n" + "AO3 FEED: " + ao3_feed + "\n" + "AO4 FEED: " + ao4_feed + "\n" + "AO5 FEED: " + ao5_feed + "\n" + "AO6 FEED: " + ao6_feed + "\n" + "AO7 FEED: " + ao7_feed + "\n" + "AO8 FEED: " + ao8_feed + "\n" +
  "DF1 SETTING: " + df1ReadValue + "\n" + "DF2 SETTING: " + df2ReadValue + "\n" + "DF3 SETTING: " + df3ReadValue + "\n" + "DF4 SETTING: " + df4ReadValue + "\n" + "DF5 SETTING: " + df5ReadValue + "\n" + "DF6 SETTING: " + df6ReadValue + "\n" + "DF7 SETTING: " + df7ReadValue + "\n" + "DF8 SETTING: " + df8ReadValue + "\n" +
  "AF1 SETTING: " + af1ReadValue + "\n" + "AF2 SETTING: " + af2ReadValue + "\n" + "AF3 SETTING: " + af3ReadValue + "\n" + "AF4 SETTING: " + af4ReadValue + "\n" + "AF5 SETTING: " + af5ReadValue + "\n" + "AF6 SETTING: " + af6ReadValue + "\n" + "AF7 SETTING: " + af7ReadValue + "\n" + "AF8 SETTING: " + af8ReadValue + "\n" +
  "HEARTBEAT UNITS: " + heartbeatSubscriptionsString + "\nEND CONFIG";

  mqttClient.beginMessage(config_dump_feed, payload.length(), retained, qos, duplicateMqttMessage);
  mqttClient.print(payload);
  mqttClient.endMessage();
  delay(50);
}
...
void publishModemSerialNumber() {
  ...
  mqttClient.beginMessage(modem_sn_feed, payload.length(), retained, qos, duplicateMqttMessage);
  mqttClient.print(payload);
  mqttClient.endMessage();
  delay(50);
}
...
void publishModemRSSI() {
  ...
  payload = signalStrengthString;

  mqttClient.beginMessage(modem_rssi_feed, payload.length(), retained, qos, duplicateMqttMessage);
  mqttClient.print(payload);
  mqttClient.endMessage();
  delay(50);
}

void publishLogUpload(bool final) {
  //M5.Lcd.fillRect(10, 200, 300, 50, BLACK);
  //M5.Lcd.drawString("PUBLISHING DO1!", 10, 200, 1);
  if (final) {
    payload = "4";
  ...
  if (sentLogFTP) {
    payload = "LOG UPLOAD SUCCESSFUL!";
  }

  mqttClient.beginMessage(log_upload_feed, payload.length(), retained, qos, duplicateMqttMessage);
  mqttClient.print(payload);
  mqttClient.endMessage();
  delay(50);
}

void publishLogDelete(bool final) {
  //M5.Lcd.fillRect(10, 200, 300, 50, BLACK);
  //M5.Lcd.drawString("PUBLISHING DO1!", 10, 200, 1);
  if (final) {
    payload = "4";
  ...
  if (deletedLogFTP) {
    payload = "LOG DELETE SUCCESSFUL!";
  }

  mqttClient.beginMessage(log_delete_feed, payload.length(), retained, qos, duplicateMqttMessage);
  mqttClient.print(payload);
  mqttClient.endMessage();
  delay(50);
}
...
void publishLogActive() {
  ...
  if (loggingEnabled) {
    payload = "DATA LOGGING ENABLED!";
  } else {
    payload = "DATA LOGGING DISABLED!";
  }
...
void publishSparkplugBAck() {
  ...
  mqttClient.beginMessage(sparkplugb_feedACK, payload.length(), retained, qos, duplicateMqttMessage);
  mqttClient.print(payload);
  mqttClient.endMessage();
  delay(50);
}
...
bool publishSparkplugBMessage(const char* topic, uint8_t* buffer, size_t buffer_length) {
  sentSparkplugBMessage = true;

  mqttClient.beginMessage(topic, buffer_length, sparkplugRetained, qos, duplicateMqttMessage);
  mqttClient.write(buffer, buffer_length);
  return mqttClient.endMessage();
}

bool publishSparkplugBLastWill(const char* topic, uint8_t* buffer, size_t buffer_length) {
  mqttClient.beginWill(topic, buffer_length, sparkplugRetained, qos);
  mqttClient.write(buffer, buffer_length);
