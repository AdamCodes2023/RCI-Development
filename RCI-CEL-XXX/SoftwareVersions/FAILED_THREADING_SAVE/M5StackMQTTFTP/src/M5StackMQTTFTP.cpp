...
//DisplayManagementThread displayThread;
//InternalManagementThread updateThread;
//bool displayThreadReady = false;
//bool updateThreadReady = true;
...
String versionNumber = String("V 2.2A");
...
/*
m5::Button_Class leftRed = M5.BtnA;
m5::Button_Class centerRed = M5.BtnB;
m5::Button_Class rightRed = M5.BtnC;
*/
...
// Create the SdFs object (handles FAT16/FAT32/exFAT automatically)
SdFs sd;
FsFile myFile;
...
void setup() {
  ...
  // Initialize M5Unified with default configurations
  // This dynamically boots the AXP192 or AXP2101 power management units
  auto cfg = M5.config();
  M5.begin(cfg);
  ...
  M5.Speaker.begin();
  M5.Speaker.setVolume(REGULAR_SPEAKER_VOLUME);
  ...
  for (auto gpio : (const uint8_t[]){18, 38, 23}) { *(volatile uint32_t*)(GPIO_PIN_MUX_REG[gpio]) |= FUN_DRV_M; gpio_pulldown_dis((gpio_num_t)gpio); gpio_pullup_en((gpio_num_t)gpio); }

  // FORCE THE DISPLAY TO RELEASE THE SPI BUS
  // We explicitly set the LCD CS pin as an output and drive it HIGH (disabled)
  //pinMode(LCD_CS, OUTPUT);
  //digitalWrite(LCD_CS, HIGH); 
  //delay(50); // Give the hardware a moment to settle

  // Manually spin up the SPI bus with Core2's unique MISO pin (38)
  SPI.begin(SD_SCK, SD_MISO, SD_MOSI, SD_CS);

  // Configure SdFat settings: 
  // - Use the SPI pins we just configured
  // - Set a safe shared bus speed (16 MHz or 10 MHz)
  // - Enable dedicated SPI to prevent bus noise
  SdSpiConfig spiConfig(SD_CS, SHARED_SPI, SD_SCK_MHZ(10), &SPI);
  ...
  *configItems[24] = "NAME: NONE";
  ...
  // Initialize the SD card at a safe 20MHz clock speed
  if (!sd.begin(spiConfig)) {
    Serial.println("SdFat initialization failed!");
    // SdFat has an amazing built-in error diagnostic tool:
  ...
  if (cardType == 0) {
    Serial.println("No SD card physically detected.");
  }
  ...
  if (sd.exists("/internal.txt") == 1) {
    myFile = sd.open("/internal.txt", O_RDONLY);
  ...
  if (sd.exists("/api.txt") == 1) {
    myFile = sd.open("/api.txt", O_RDONLY);
    while (myFile.available()) {
      analyzeSDCardContents();
    }
    myFile.close();
  ...
  if (sd.exists("/creds.txt") == 1) {
    myFile = sd.open("/creds.txt", O_RDONLY);
    while (myFile.available()) {
      analyzeSDCardContents();
    }
    myFile.close();
  ...
  if (sd.exists(sdConfigFileName) == 1) {
    myFile = sd.open(sdConfigFileName, O_RDONLY);
  ...
  */
  ...
  switch (IONum.toInt()) {
    case 1:
    case 2:
      rciTitle = String("RCI-200-CEL");
      break;

    case 3:
    case 4:
      rciTitle = String("RCI-400-CEL");
      break;
    ...
    case 7:
    case 8:
      rciTitle = String("RCI-800-CEL");
      break;

    default:
      rciTitle = String("RCI-000-CEL");
      break;
  }
  ...
  */
  ...
  myFile = sd.open(sdConfigFileName, O_RDONLY | O_CREAT);
  if (myFile.available() < 1) {
    myFile.close();
  ...
  myFile = sd.open(sdConfigFileName, O_WRONLY | O_CREAT | O_TRUNC);
  ...
  *(sparkplugNode->vars.scan_rate_tag_value) = updateTime.toInt();
  ...
  //displayThread.SetStackSize(32 * 1024); // 32KB
  //updateThread.SetStackSize(32 * 1024);  // 32KB

  //updateThread.Initialize();  //Core 1
  //displayThread.Initialize(); //Core 0
...
void loop() {
  ...
  if (normalMode && mqttClient.connected()) {
    Ethernet.maintain();
