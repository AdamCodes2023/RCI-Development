...
void backgroundReceiveCellularSignalQualityGLIMSPE(bool displayBars) {
  if (signalClient.connected()) {
    ...
    if (statusString.length() > 0) {
      signalQualityString = statusString.substring(statusString.indexOf("signalquality") + 17, statusString.indexOf(",", statusString.indexOf("signalquality") + 17) - 1);
      signalStrengthString = statusString.substring(statusString.indexOf("currssi") + 11, statusString.indexOf(",", statusString.indexOf("currssi") + 11) - 1);
    }
