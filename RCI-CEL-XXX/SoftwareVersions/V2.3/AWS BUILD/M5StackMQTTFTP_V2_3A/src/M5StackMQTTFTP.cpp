...
#include "cellular/cellular.cpp"
#include "certificates/certificates.cpp"  // This file must be regenerated.
#include "config/config.cpp"
#include "cycling/cycling.cpp"
#include "fwupdate/fwupdate.cpp"
#include "implementftp/implementftp.cpp"
#include "implementmqtt/implementmqtt.cpp"
#include "io/io.cpp"
...
String config_dump_feed = String("");
String modem_sn_feed = String("");
String modem_rssi_feed = String("");
String log_upload_feed = String("");
String log_delete_feed = String("");
String sparkplugb_feed = String("");
String sparkplugb_ncmd_feed = String("");
String sparkplugb_ndata_feed = String("");
String sparkplugb_all_feed = String("");

String sparkplugb_feedACK = String("");
...
bool receivedConfigFTP = false;
bool receivedCredsFTP = false;
bool receivedInternalFTP = false;
bool receivedApiFTP = false;
...
bool publishConfigDumpFlag = false;
bool publishModemSerialNumberFlag = false;
bool publishModemRSSIFlag = false;
...
String versionNumber = String(RCI_FIRMWARE_VERSION);
String statusString = String("");
String signalQualityString = String("NR");
String signalStrengthString = String("NR");
String lastLoggedSignalStrengthString = String("NR");
String signalStatusString = String("NA");
...
const short continuousUpdateComponentsInterval = 500;
...
const long long getTimeInterval = 86400000 / 4;
...
const long long requestSignalQualityNormalModeInterval = 60000;
...
const long long extendedSentSparkplugBMessageRetryInterval = 120000;
const long long logCellularSignalDataInterval = 3600000;
const short cyclingFreezeInterval = 5000;
...
unsigned long long continuousUpdateComponentsPreviousMillis = 0;
...
unsigned long long extendedSentSparkplugBMessageRetryPreviousMillis = 0;
unsigned long long logCellularSignalDataPreviousMillis = 0;
...
unsigned long long continuousUpdateComponentsCurrentMillis = 0;
...
unsigned long long extendedSentSparkplugBMessageRetryCurrentMillis = 0;
unsigned long long logCellularSignalDataCurrentMillis = 0;

bool commFailToggledOnce = true;
bool clear = false;
bool screenRefresh = true;
...
bool leftCyclingFrozenHeld = false;
...
SSLClient ethClientSSL(ethClient, TAs, (size_t)TAs_NUM, G36, 1, SSLClient::SSL_ERROR);
//SSLClient ethClientSSL(ethClient, nullptr, 0, G36);
//SSLClient ethClientSSL(nullptr);
MqttClient mqttClient(ethClientSSL);
//MqttClient mqttClient(nullptr);

//HISTORIAN CLIENT HERE!


SparkplugNodeConfig* sparkplugNode = NULL;
SparkplugNodeState sparkplugNodeState;
size_t payload_buffer_size = int(pow(2.0, 16.0));

bool receivedSparkplugBMessage = false;
bool sentSparkplugBMessage = false;
short sentSparkplugBMessageLoopCount = 0;
short sentSparkplugBMessageRetries = 0;
bool extendSentSparkplugBMessageRetry = false;

//OUTPUT TAGS
int32_t pcfw1Out1Recv = 0;
int32_t pcfw1Out2Recv = 0;
int32_t pcfw1Out3Recv = 0;
int32_t pcfw1Out4Recv = 0;
int32_t pcfw1Out5Recv = 0;
int32_t pcfw1Out6Recv = 0;
int32_t pcfw1Out7Recv = 0;
int32_t pcfw1Out8Recv = 0;

int16_t dac1Out1Recv = 0;
int16_t dac1Out2Recv = 0;
int16_t dac1Out3Recv = 0;
int16_t dac1Out4Recv = 0;
int16_t dac2Out1Recv = 0;
int16_t dac2Out2Recv = 0;
int16_t dac2Out3Recv = 0;
int16_t dac2Out4Recv = 0;
...
const char* tzInfo = "EST5EDT,M3.2.0,M11.1.0"; // Adjust to your local timezone
const char* lastSavedMonthKey = "LogMonth";
const char* lastSavedYearKey = "LogYear";
int lastSavedMonthTracker = -1;
int lastSavedYearTracker = -1;
...
bool sdStorageSuccess = true;
bool configReadSuccess = true;
...
const char* configStringKey = "ConfigString";
...
const char* publishConfigPrefKey = "PubConf";
const char* publishCredsPrefKey = "PubCreds";
const char* publishInternalPrefKey = "PubInternal";
const char* publishApiPrefKey = "PubApi";
const char* loggingEnabledKey = "LogEN";
...
String ftpLogWorkingDirectory = String("");
...
String updateTime = String("10000");
...
bool logDisconnect = true;

bool sentLogFTP = false;
bool logUploadStep1 = false;
bool logUploadStep1PubOnce = false;
bool logUploadStep2 = false;
bool logUploadStep2PubOnce = false;
bool logUploadStep3 = false;

bool deletedLogFTP = false;
bool logDeleteStep1 = false;
bool logDeleteStep1PubOnce = false;
bool logDeleteStep2 = false;
bool logDeleteStep2PubOnce = false;
bool logDeleteStep3 = false;

bool loggingEnabled = false;
bool publishLoggingStatus = false;
...
void setup() {
  ...
  *configItems[22] = "RSSI: " + signalStrengthString;
  ...
  *configItems[24] = "NAME: NONE";
  ...
  configString = preferences.getString(configStringKey);
  ...
  receivedConfigFTP = preferences.getBool(publishConfigPrefKey);
  receivedCredsFTP = preferences.getBool(publishCredsPrefKey);
  receivedInternalFTP = preferences.getBool(publishInternalPrefKey);
  receivedApiFTP = preferences.getBool(publishApiPrefKey);
  loggingEnabled = preferences.getBool(loggingEnabledKey);
  preferences.end();
  ...
  for (int i = 0; i < configString.length(); i++) {
    String character = configString.substring(i, i + 1);
    if (character.equalsIgnoreCase(";")) {
      analyzeFTPFileContents(configKeyword);
      configKeyword = "";
    }
    else {
      configKeyword += character;
    }
  }
  ...
  */
  ...
  if (!SD.begin(TFCARD_CS_PIN, SPI, 4000000)) {
    sdStorageSuccess = false;
    errorCode = "ERROR CODE: 17";
  }

  if (configString.isEmpty()) {
    configReadSuccess = false;
    errorCode = "ERROR CODE: 18";
  }
  ...
  switch (IONum.toInt()) {
    case 1:
    case 2:
      rciTitle = String("RCI-200-") + String(RCI_MODEL_TYPE);
      break;

    case 3:
    case 4:
      rciTitle = String("RCI-400-") + String(RCI_MODEL_TYPE);
      break;
    ...
    case 7:
    case 8:
      rciTitle = String("RCI-800-") + String(RCI_MODEL_TYPE);
      break;

    default:
      rciTitle = String("RCI-000-") + String(RCI_MODEL_TYPE);
      break;
  }
  ...
  */
  ...
  sparkplugNode = createSparkplugNode(groupID.c_str(), unitID.c_str(), payload_buffer_size, timestampFunction);
  if (sparkplugNode == NULL) {
    Serial.println("FAILED TO CREATE SPARKPLUG NODE");
  }

  // Change scan rate to updateTime seconds
  *(sparkplugNode->vars.scan_rate_tag_value) = updateTime.toInt();

  // Print the newly instantiated node
  printSparkplugNodeConfig(sparkplugNode);
...
void loop() {
  ...
  if (normalMode) {
    ...
    continuousUpdateComponentsCurrentMillis = millis();
    if (continuousUpdateComponentsCurrentMillis - continuousUpdateComponentsPreviousMillis >= continuousUpdateComponentsInterval) {
      pollForChannelChanges();
      continuousUpdateComponentsPreviousMillis = continuousUpdateComponentsCurrentMillis;
    }

    cyclingPauseCurrentMillis = millis();
    if ((cyclingPauseCurrentMillis - cyclingPausePreviousMillis >= cyclingPauseInterval) && cyclingPaused && !cyclingFrozen) {
      cyclingPaused = false;
    ...
    extendedSentSparkplugBMessageRetryCurrentMillis = millis();
    if (checkForSubscribeCurrentMillis - checkForSubscribePreviousMillis >= checkForSubscribeInterval) {
      ...
      */
      ...
      if (sentSparkplugBMessage) {
        sentSparkplugBMessageLoopCount++;
        if ((sentSparkplugBMessageLoopCount > NORMAL_MAX_SPARKPLUGB_MESSAGE_LOOP_COUNT && !extendSentSparkplugBMessageRetry) || (sentSparkplugBMessageLoopCount > EXTENDED_MAX_SPARKPLUGB_MESSAGE_LOOP_COUNT && extendSentSparkplugBMessageRetry)) {
          sentSparkplugBMessageLoopCount = 0;
          *(sparkplugNode->vars.rebirth_tag_value) = true;
          sentSparkplugBMessageRetries++;
        }
        if (sentSparkplugBMessageRetries >= NORMAL_MAX_SPARKPLUGB_MESSAGE_RETRIES && !extendSentSparkplugBMessageRetry) {
          sentSparkplugBMessageRetries = 0;
          extendSentSparkplugBMessageRetry = true;
        }
        if (sentSparkplugBMessageRetries >= EXTENDED_MAX_SPARKPLUGB_MESSAGE_RETRIES && extendSentSparkplugBMessageRetry) {
          *(sparkplugNode->vars.rebirth_tag_value) = false;
    ...
    if (receivedSparkplugBMessage) {
      receivedSparkplugBMessage = false;
      publishSparkplugBAck();
    }
    ...
    if (publishConfigDumpFlag) {
      publishConfigDump();
      publishConfigDumpFlag = false;
    }

    if (publishModemSerialNumberFlag) {
      publishModemSerialNumber();
      publishModemSerialNumberFlag = false;
    }

    if (publishModemRSSIFlag) {
      publishModemRSSI();
      publishModemRSSIFlag = false;
    }

    if (logUploadStep1PubOnce) {
      publishLogUpload(false);
      logUploadStep1PubOnce = false;
    }

    if (logUploadStep2PubOnce) {
      publishLogUpload(true);
      logUploadStep2PubOnce = false;
    }

    if (logDeleteStep1PubOnce) {
      publishLogDelete(false);
      logDeleteStep1PubOnce = false;
    }

    if (logDeleteStep2PubOnce) {
      publishLogDelete(true);
      logDeleteStep2PubOnce = false;
    }

    if (receivedConfigFTP) {
      publishConf(false);
      receivedConfigFTP = false;
      preferences.begin(pnamespace);
    ...
    if (publishLoggingStatus) {
      publishLogActive();
      publishLoggingStatus = false;
      preferences.begin(pnamespace);
      preferences.putBool(loggingEnabledKey, loggingEnabled);
      preferences.end();
    }
    ...
    */
    ...
    manageSparkplugNodeState();
    updateOutputsFromTags();
