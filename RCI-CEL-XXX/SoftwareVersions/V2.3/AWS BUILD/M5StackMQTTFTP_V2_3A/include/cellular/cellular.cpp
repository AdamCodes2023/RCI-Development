...
void backgroundReceiveCellularSignalQuality(bool displayBars) {
  if (signalClient.connected()) {
    ...
    if (signalStrengthString.toInt() != 0) {
      *configItems[22] = "RSSI: " + signalStrengthString + " dBm";
    } else {
    ...
    Serial.println("RSSI: " + signalStrengthString + " dBm");
...
void requestCellularSignalQualityGLIMPSE() {
  ...
  // connect to cell modem on port 8888:
  if (signalClient.connect(HOST_NAME, HTTP_PORT)) {
    // if connected:
    Serial.println("Connected to server");
    // make a HTTP request:
    // send HTTP header
    signalClient.println(HTTP_METHOD + " " + PATH_NAME + " HTTP/1.1");
    signalClient.println("Host: " + String(HOST_NAME));
    signalClient.println("Connection: close");
    signalClient.println(); // end HTTP header
  } else {// if not connected:
    Serial.println("connection failed");
  }
}

void backgroundReceiveCellularSignalQualityGLIMSPE(bool displayBars) {
  if (signalClient.connected()) {
    if (signalClient.available()) {
      // read an incoming byte from the server and print it to serial monitor:
      char c = signalClient.read();
    ...
    if (statusString.length() > 0) {
      signalQualityString = statusString.substring(statusString.indexOf("signalquality") + 16, statusString.indexOf(",", statusString.indexOf("signalquality") + 16) - 1);
      signalStrengthString = statusString.substring(statusString.indexOf("currssi") + 10, statusString.indexOf(",", statusString.indexOf("currssi") + 10) - 1);
    }
    ...
    if (displayBars && (calculateCellularSignalQualityBars(signalStrengthString.toInt()) != calculateCellularSignalQualityBars(previousSignalStrengthString.toInt()))) {
      displayCellularSignalQuality();
    }
  }
}
...
void signalQualityRoutine() {
  ...
  */
  ...
  if (optRadioSN.equalsIgnoreCase("")) {
    return;
  }

  //OPTCONNECT GLIMPSE API ROUTINE
  requestSignalQualityCurrentMillis = millis();
  if ((cfgServerMode && requestSignalQualityCurrentMillis - requestSignalQualityPreviousMillis >= requestSignalQualityConfigModeInterval) || (normalMode && requestSignalQualityCurrentMillis - requestSignalQualityPreviousMillis >= requestSignalQualityNormalModeInterval)) {
    requestSignalQualityPreviousMillis = requestSignalQualityCurrentMillis;
    requestCellularSignalQualityGLIMPSE();
  }

  if (onSignalClientDisconnect) {
    backgroundReceiveCellularSignalQualityGLIMSPE(normalMode);
  }

  logCellularSignalDataCurrentMillis = millis();
  if (!(signalStrengthString.toInt() - lastLoggedSignalStrengthString.toInt() < 5 && signalStrengthString.toInt() - lastLoggedSignalStrengthString.toInt() > -5) || (logCellularSignalDataCurrentMillis - logCellularSignalDataPreviousMillis >= logCellularSignalDataInterval)) {
    logCellularSignalDataPreviousMillis = logCellularSignalDataCurrentMillis;
    logSignalQualityEvent();
    lastLoggedSignalStrengthString = signalStrengthString;
  }
