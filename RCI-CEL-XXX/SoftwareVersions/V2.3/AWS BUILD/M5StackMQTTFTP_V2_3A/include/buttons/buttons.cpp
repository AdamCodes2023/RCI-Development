...
void onLeftPress() {
  if (M5.BtnA.pressedFor(cyclingFreezeInterval) && normalMode && !cyclingFrozen && !leftCyclingFrozenHeld) {
    M5.Spk.PlaySound(freezeBeep, sizeof(freezeBeep));
    leftCyclingFrozenHeld = true;
  }
...
void onLeftRelease() {
  if (M5.BtnA.wasReleased() && leftCyclingFrozenHeld) {
