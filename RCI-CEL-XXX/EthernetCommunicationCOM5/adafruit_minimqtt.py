...
def set_socket(sock, iface=None) -> None:
    ...
    _default_sock = sock
    if iface:
        _default_sock.set_interface(_default_sock, iface)
...
class _FakeSSLSocket:
    ...
    def __init__(self, socket, tls_mode) -> None:
        ...
        self._mode = tls_mode
        self.family: int = AF_INET,
        self.type: int = SOCK_STREAM,
        self.settimeout = self._socket.setdefaulttimeout
        self.send = sock.send
        self.recv = sock.recv
...
class _FakeSSLContext:
    ...
    def wrap_socket(self, socket, server_hostname=None) -> _FakeSSLSocket:
        ...
        return _FakeSSLSocket(socket, const(0x03))
...
class MQTT:
    ...
    def _get_connect_socket(self, host: str, port: int, *, timeout: int = 1):
        ...
        addr_info = self._socket_pool.getaddrinfo(self._socket_pool,
            _host = host, _port = port, _family = 0, _type = self._socket_pool.SOCK_STREAM
        )[0]

        try:
            #sock = self._socket_pool.socket(addr_info[0], addr_info[1])
            sock = self._socket_pool
        ...
        #sock._socket._default_socket_timeout = _timeout

        last_exception = None
        #sock.close(sock)
        sock._socket_closed = False
        try:
            sock.connect(sock, (connect_host, port))
    ...
    def _connect(
        ...
        self._sock.send(self, fixed_header)
        self._sock.send(self, var_header)
        # [MQTT-3.1.3-4]
    ...
    def _sock_exact_recv(self, bufsize: int) -> bytearray:
        ...
        is_esp32spi_sock = False
        if not is_esp32spi_sock and not self._backwards_compatible_sock:
            # CPython/Socketpool Impl.
            rc = bytearray(bufsize)
            mv = memoryview(rc)
            recv_len = self._sock.recv_into(self._sock, rc, bufsize)
            ...
            mv = mv[recv_len:]
            while to_read > 0:
                recv_len = self._sock.recv_into(self._sock, mv, to_read)
                to_read -= recv_len
                mv = mv[recv_len:]
    ...
    def _send_str(self, string: str) -> None:
        ...
        if isinstance(string, str):
            self._sock.send(self, struct.pack("!H", len(string.encode("utf-8"))))
            self._sock.send(self, str.encode(string, "utf-8"))
        else:
            self._sock.send(self, struct.pack("!H", len(string)))
            self._sock.send(self, string)
