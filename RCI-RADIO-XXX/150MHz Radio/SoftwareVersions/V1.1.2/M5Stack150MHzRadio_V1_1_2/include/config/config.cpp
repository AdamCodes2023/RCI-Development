...
void analyzeBLEWriteContents(String topic) {
  ...
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("UID")) {
    ...
    *configItems[3] = "UID: " + unitID;
    if (unitID.equalsIgnoreCase(MESSAGING_HOST_UNIT_ID)) {
      isHost = true;
    }
  }
  ...
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("UPT")) {
    ...
    *configItems[21] = "UPDATE: " + updateTime;
    publishAllInterval = updateTime.toInt();
  ...
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("DO1")) {
    ...
    *configItems[5] = "DO1: NONE";
    if (topic.substring(0, topic.length() - 4).indexOf("_DI0") < 0) {
  ...
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("AO1")) {
    ...
    *configItems[13] = "AO1: NONE";
    if (topic.substring(0, topic.length() - 4).indexOf("_AI0") < 0) {
  ...
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("RFP")) {
    radioModemSelectedRFPower = topic.substring(0, topic.length() - 4);
    *configItems[23] = "RPOW:" + radioModemSelectedRFPower + "W RCHA:" + radioModemSelectedChannel;
  }
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("RCH")) {
    radioModemSelectedChannel = topic.substring(0, topic.length() - 4);
    *configItems[23] = "RPOW:" + radioModemSelectedRFPower + "W RCHA:" + radioModemSelectedChannel;
  }
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("RPH")) {
    radioModemSelectedPHP = topic.substring(0, topic.length() - 4);
    //*configItems[23] = "RPOW: " + radioModemSelectedRFPower + " RCHA: " + radioModemSelectedChannel;
  }
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("RSH")) {
    radioModemSelectedSHP = topic.substring(0, topic.length() - 4);
    //*configItems[23] = "RPOW: " + radioModemSelectedRFPower + " RCHA: " + radioModemSelectedChannel;
  }
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("RNW")) {
    radioModemSelectedNetworkID = topic.substring(0, topic.length() - 4);
    //*configItems[23] = "RPOW: " + radioModemSelectedRFPower + " RCHA: " + radioModemSelectedChannel;
  }
}
...
void analyzeSDCardContents() {
  ...
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("DO1")) {
    do1_feed = topic.substring(0, topic.length() - 4);
    if (do1_feed.length() > 1) {
      *configItems[5] = "DO1: " + do1_feed;
      collectRemoteUnitIDs(topic.substring(topic.indexOf("_") + 1, topic.indexOf("_", topic.indexOf("_") + 1)));
    }
  }
  if (topic.substring(topic.length() - 3, topic.length()).equalsIgnoreCase("DO2")) {
    do2_feed = topic.substring(0, topic.length() - 4);
    if (do2_feed.length() > 1) {
      *configItems[6] = "DO2: " + do2_feed;
      collectRemoteUnitIDs(topic.substring(topic.indexOf("_") + 1, topic.indexOf("_", topic.indexOf("_") + 1)));
    }
  }
...
void collectRemoteUnitIDs(String remoteUnitID) {
  int intRemoteUnitID = remoteUnitID.toInt();
  for (int i = 0; i < numberOfRemoteUnits; i++) {
    if (connectedRemoteUnitIDs[i] == intRemoteUnitID) {
      return;
    }
  }
  connectedRemoteUnitIDs[numberOfRemoteUnits] = intRemoteUnitID;
  numberOfRemoteUnits++;
}
