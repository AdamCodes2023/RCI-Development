...
void onLeftRelease() {
  ...
  if (warningMenu1) {
    if (M5.BtnA.wasReleased() && leftPressedTwice) {
      ...
      M5.Lcd.drawString("RADIO TEST", 50, 200, 1);
  ...
  if (configMenu1 || configMenu2) {
    if (M5.BtnA.wasReleased() && leftPressedOnce) {
      leftPressedOnce = false;
      configMenu1Iterator--;
      if (configMenu1) { // BASE SETTINGS MENU
      ...
      else if (configMenu2) { // OUTPUT TEST SUBMENU
        if (configMenu1Iterator < 0) {
          configMenu1Iterator = 4; //CHANGE TO ADJUST NUMBER OF MENUS (DO NOT FORGET TO ADJUST GREEN SELECTION SQUARE)
        }
      }
...
void onRightPress() {
  ...
  if (M5.BtnC.wasPressed() && (normalMode || cfgServerMode || radioTestMode) && !cyclingPaused) {
...
void onRightRelease() {
  ...
  if (M5.BtnC.wasReleased() && rightPressedTwice) {
    ...
    if (configMenu1Iterator == 4) {
      ...
      M5.Lcd.clear();
      M5.Lcd.setCursor(0, 0);
      M5.Lcd.drawString("RADIO TESTING", 40, 50, 1);
      M5.Lcd.drawString("TRANSMITTING ON", 20, 100, 1);
      M5.Lcd.drawString("THE SET CHANNEL", 20, 150, 1);
      M5.Lcd.drawString("ESC", 245, 210, 1);
      enterContinuousRadioTest();
      radioTestMode = true;
      radioTestModeCurrentMillis = millis();
      radioTestModePreviousMillis = radioTestModeCurrentMillis;
      //delay(5000);
    }
  }
