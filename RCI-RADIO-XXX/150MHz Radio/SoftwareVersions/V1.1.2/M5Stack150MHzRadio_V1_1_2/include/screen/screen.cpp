    ...
    if (updateSignalDeviceInfoCurrentMillis - updateSignalDeviceInfoPreviousMillis >= updateSignalDeviceInfoInterval) {
      ...
      if (!testingSignal || !isHost) {
        if (isHost) {
          ...
          int targetUnitID = connectedRemoteUnitIDs[signalCounter] - 1;
          String stringTargetUnitID = String(targetUnitID);
          String tempSSIString = String("RSSI R") + stringTargetUnitID + String(": ") + signalStrengthString;
          *configItems[22] = tempSSIString;
          //M5.Lcd.drawString("R" + stringTargetUnitID, 1, 210, 1);
        } else {
          String tempSSIString = String("RSSI H: ") + signalStrengthString;
          *configItems[22] = tempSSIString;
